...
Resilience features:
  - Mock mode (USE_MOCK_AI=true) → reads pre-written post-mortems from JSON
  - Graceful degradation         → statistical fallback if API fails
  - Low-token model              → claude-haiku by default
"""
...
def _statistical_analysis(
    ...
    negative_steps = sum(1 for r in rewards if r < 0)
    pct_negative   = negative_steps / max(len(rewards), 1)
    ...
    rec_pest    = locals().get("rec_pest",    2.0)
    rec_harvest = 1.0
...
class PostMortemAnalyst:
    ...
    Resilience hierarchy:
      ...
      2. Live API      → claude-haiku LLM call
...
- Pests caused loss → lower recommended_pest_threshold."""
    ...
    def analyse(
        ...
        """Run post-mortem analysis via mock → LLM → statistical fallback."""

        # ── Path 1: Mock ───────────────────────────────────────────────────────
        if USE_MOCK_AI and self._mock_pool:
            entry = self._mock_pool[self._mock_index % len(self._mock_pool)]
            self._mock_index += 1
            print(f"  [PostMortem] Mock response #{self._mock_index}")
        ...
        if self._client:
            try:
                ...
                print(f"  [PostMortem] API failed ({type(e).__name__}) — switching to statistical analysis.")

        # ── Path 3: Statistical fallback (always works) ────────────────────────
        print("  [PostMortem] Running statistical analysis...")
    ...
    def _build_prompt(self, episode_log, total_reward, final_score, task) -> str:
        sample = (episode_log if len(episode_log) <= 30 else
                  episode_log[:5] + episode_log[len(episode_log)//2-2:len(episode_log)//2+2] + episode_log[-5:])
        return (
